define(['aeris', 'aeris/utils', 'vendor/backbone'], function(aeris, _, Backbone) {
  ...
  _.extend(aeris.Events.prototype, Backbone.Events);


  aeris.Events.prototype.eachEventHash_ = function(eventHash, callback, ctx) {
    ctx || (ctx = this);
  ...
  aeris.Events.prototype.on = function(eventHash, ctx) {
   ...
   *                  specified topic.
   */
  aeris.Events.prototype.off = function(event, handler, ctx) {
    // Handle and normalize events hash
    if(_.isObject(event)) {
      ctx = arguments[1];
