define([
   ...
   *
   * @param {aeris.maps.layer.AerisTile} layer The layer to get the times for.
  ...
  aeris.AerisAPI.prototype.getTileTimes = function(layer, opt_refresh) {
    ...
    var promise = this.timePromises_[layer.get('tileType')];
    if (!promise || refresh) {
      promise = this.timePromises_[layer.get('tileType')] = new aeris.Promise();
      var url = 'http://tile.aerisapi.com/' +
                config.get('apiId') + '_' +
                config.get('apiSecret') + '/' +
                layer.get('tileType') + '.jsonp';
      aeris.jsonp.get(url, {}, function(data) {
        var times = [];
        var length = data.files.length;
        for (var i = 0; i < length; i++) {
          times.push(data.files[i].timestamp * 1000);   // Timestamp is in seconds, JS uses milliseconds
        }
   ...
   *
   * @param {aeris.maps.layers.Layer} layer The layer to listen for time updates.
  ...
  aeris.AerisAPI.prototype.onTileTimesUpdate = function(layer, fn, opt_ctx) {
    this.on('tileTimesUpdated_' + layer.cid, fn, opt_ctx);
   ...
   *
   * @param {aeris.maps.layers.Layer} layer The layer to unsubscribe for time updates.
  ...
  aeris.AerisAPI.prototype.offTileTimesUpdate =
    ...
    var count = this.off('tileTimesUpdated_' + layer.cid, opt_fn, opt_ctx);
    if (count === 0) {
      window.clearInterval(this.tileTimesUpdates_[layer.cid]);
      this.tileTimesUpdates_[layer.cid] = null;
    }
    return count;
  };
   ...
   * Get the times for a specific layer.
   *
   * @param {aeris.maps.layers.Layer} layer The layer to get the times for.
   * @param {boolean=} opt_refresh Optionally force a refresh of the times.
   * @return {aeris.Promise}
   */
