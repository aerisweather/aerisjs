define([
  'ai/util',
  'ai/promise',
  'ai/geolocate/options/html5serviceoptions',
  ...
  HTML5GeolocateService.prototype.getCurrentPosition = function() {
    ...
    if (!this.isSupported()) {
      promise.reject(this.createGeolocateError_({
      ...
      this.navigator_.geolocation.getCurrentPosition(
        ...
        function(err) {
          promise.reject(self.createGeolocateError_(err));
        },
  ...
  HTML5GeolocateService.prototype.watchPosition = function(onSuccess, onError, opt_options) {
    ...
    if (!this.isSupported()) {
      onError(this.createGeolocateError_({
        message: 'HTML5 Geolocation is not available.',
        code: GeolocateServiceError.POSITION_UNAVAILABLE
      }));
    }
