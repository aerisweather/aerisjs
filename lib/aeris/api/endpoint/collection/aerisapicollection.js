define([
  ...
  var AerisApiCollection = function(opt_models, opt_options) {
    var options = _.extend({
      ...
      action: '',
      params: {},
      api: null,     // default is aeris.AerisAPI.
      throttle: 2000,
      server: 'http://api.aerisapi.com/'
    }, opt_options);


    // Throttle the sync method
    // so that requests will only
    // be made every [options.throttle] milliseconds.
    //
    // This can be useful if you have rapidly-firing events
    // bound to the `fetch` method.
    if (options.throttle > 0) {
      this.sync = _.throttle(this.sync, options.throttle);
    }
  ...
  AerisApiCollection.prototype.sync = function(method, model, opt_options) {
    ...
    this.jsonp_.get(url, data, function(data) {
      if (!data.success) {
        promise.reject(data);
      }
      else {
