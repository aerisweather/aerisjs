define([
  ...
  aeris.maps.gmaps.route.RouteBuilder = function(opt_options) {
    ...
    this.routeEvents_ = {
      ...
      change: function(waypoint, evtObj) {
        var interests = ['latLon', 'geocodedLatLon', 'path'];
        if (_.intersection(evtObj.properties, interests)) {
  ...
  aeris.maps.gmaps.route.RouteBuilder.prototype.delegateRouteEvents = function(opt_events, opt_ctx) {
    ...
    this.getRoute().bindEvents(opt_events, this.getRoute(), opt_ctx);
  };
   ...
   * @param {Object=} opt_events Defaults to this.routeEvents_.
   *        eg { 'click': this.handleClick_ }.
   * @param {Object} opt_ctx Context to match for undbinding.
   */
  aeris.maps.gmaps.route.RouteBuilder.prototype.undelegateRouteEvents = function(opt_events, opt_ctx) {
    opt_events || (opt_events = this.routeEvents_);
    opt_ctx || (opt_ctx = this);

    this.getRoute().unbindEvents(opt_events, this.getRoute(), opt_ctx);
  };
