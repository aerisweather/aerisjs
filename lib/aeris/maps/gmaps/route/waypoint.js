define([
...
], function(_, Marker, PathValidator, Polyline, GoogleDirectionsService, NonstopDirectionsService, JSONParseError, ValidationError) {
  /**
  ...
  Waypoint.prototype.setPathStartsAt = function(fromLatLon) {
    var directionsService = this.getDirectionsService_();
    var toLatLon = this.getPosition();

    var promise = directionsService.fetchPath(fromLatLon, toLatLon, {
  ...
  Waypoint.prototype.triggerDirectionsServiceError_ = function(directionsResults) {
    this.trigger('directions:error', directionsResults);
  };
