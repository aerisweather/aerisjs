define([
   ...
   * @mixes {aeris.Events}
   */
  var Waypoint = function(opt_options) {
   ...
   * @param {Object} attrs
   */
  Waypoint.prototype.set = function(attrs, opt_options) {
    var illegals = ['cid'];
    var options = _.extend({
      trigger: true
    }, opt_options);
  ...
  Waypoint.prototype.toJSON = function() {
    ...
    if (path && path.length && path[0].lat) {
  ...
  Waypoint.prototype.reset = function(obj) {
    var distance = obj instanceof Waypoint ? obj.getDistance() : obj.distance;
