define([
  ...
  var Waypoint = function(opt_attrs, opt_options) {
    ...
    this.listenTo(this, {
      ...
      // Reset geocodedLatLon when position changes
      // (geocoded will need to be recalculated).
      'change:position': function(model, latLon, opts) {
        // First, check that geocodedLatLon isn't being
        // directly set as part of this change event
        if (!model.changedAttributes().geocodedLatLon) {
          this.set('geocodedLatLon', null, { validate: true });
        }
      },


      // Update the position to match
      // the (more precise) geocodedLatLon
      'change:geocodedLatLon': function(model, latLon, opts) {
        if (latLon) {
