define([
...
], function(_, Collection, Promise, WaypointNotInRouteError, InvalidArgumentError, JSONParseError) {
  /**
  ...
  /** @return {Array.<aeris.maps.extension.MapObjectInterface>} */
  ...
  Route.prototype.updatePathsForExistingWaypoint_ = function(waypoint) {
    ...
    if (!this.contains(waypoint)) {
      throw new WaypointNotInRouteError('Cannot update paths: Expected waypoint to be ' +
        'in the route, but it is not.');
    }
