...
function(aeris, Events, AerisMap, Waypoint, Route, mapUtils, InvalidArgumentError) {
  ...
  aeris.maps.gmaps.route.RouteRenderer = function(aerisMap, opt_options) {
    ...
    if (options.styles) {
      _.extend(this.styles, options.styles);
    }
  ...
  aeris.maps.gmaps.route.RouteRenderer.prototype.drawIcon_ = function(latLon) {
    var icon = new aeris.maps.markers.Icon(
      latLon,
      aeris.config.path + 'assets/marker_yellow.png',
      20, 20,
      {
        clickable: true,
        draggable: true
