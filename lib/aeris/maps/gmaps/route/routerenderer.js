...
function(_, config, Events, Waypoint, Route, mapUtils, GoogleEvents, InvalidArgumentError) {
  /**
  ...
  RouteRenderer.prototype.renderWaypoint = function(waypoint, route) {
    ...
    if (waypoint.get('path') && waypoint.get('path').length) {
      polyline = this.renderPath(
        waypoint.get('path'),
        waypoint.get('followDirections') ? this.pathOptions_ : this.offPathOptions_
      );

      this.bindPolylineEvents_(polyline, waypoint);
    }
  ...
  /**
   * Bind events to the polyline.
   *
   * @param {google.maps.Polyline} polyline
   * @param {aeris.maps.gmaps.route.Waypoint} waypoint
   * @private
   */
  RouteRenderer.prototype.bindPolylineEvents_ = function(polyline, waypoint) {
    ...
    this.googleEvents_.listenTo(polyline, {
      'mousedown': function(evt) {
        var self = this;

        this.map_.getView().setOptions({ draggable: false });
        this.trigger('path:dragstart', mapUtils.latLngToArray(evt.latLng), waypoint);
        evt.stop();

        // Wait for mouseup on map
        // Because we're no longer mousing on the path
        google.maps.event.addListenerOnce(this.map_.getView(), 'mouseup', function(evt) {
          self.map_.getView().setOptions({ draggable: true });
          self.trigger('path:dragend', mapUtils.latLngToArray(evt.latLng), waypoint);
          evt.stop();
        });
      },
      'click': function(evt) {
   ...
   */
  RouteRenderer.prototype.renderMarker = function(waypoint, opt_options) {
    var options = opt_options || {};
  ...
  RouteRenderer.prototype.eraseWaypoint = function(waypoint, route) {
    ...
    polyline = this.routeViews_[routeId][waypointId].path;
    ...
    if (polyline) {
      ...
      this.googleEvents_.stopListening(polyline);
    }
