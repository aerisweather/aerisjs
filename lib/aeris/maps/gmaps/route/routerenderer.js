define(['aeris', 'base/extension/mapextension', 'base/markers/icon'],
function(aeris) {
  ...
  aeris.maps.gmaps.route.RouteRenderer.prototype.setRoute = function(route) {
    ...
    this.route_.on('add', this.renderWaypoint, this);
   ...
   *
   * @param {aeris.maps.gmaps.route.Waypoint} waypoint The Waypoint to display
   *     the Icon for.
   */
  aeris.maps.gmaps.route.RouteRenderer.prototype.drawIcon = function(waypoint) {
    var icon = new aeris.maps.markers.Icon(waypoint.getLatLon(),
        aeris.config.path + 'assets/wx-details-marker.png', 20, 20,
        {
          click: this.handleIconClick_(this.route_, waypoint)
        });
  ...
  /**
   * Create a callback function for icon clicks and bubble the event by
   * triggering a waypoint click on the route..
   *
   * @fires aeris.maps.gmaps.route.Route#click:waypoint
   * @param {aeris.maps.gmaps.route.Route} route The Route to handle
   *     and bubble the waypoint click event for.
   * @param {aeris.maps.gmaps.route.Waypoint} waypoint The Waypoint that is
   *     being clicked.
   * @return {Function}
   */
  aeris.maps.gmaps.route.RouteRenderer.prototype.handleIconClick_ =
      function(route, waypoint) {
    function fn(event) {
      route.trigger('click:waypoint', waypoint);
    };
    return fn;
  };
  ...
  aeris.maps.gmaps.route.RouteRenderer.prototype.clearListeners_ = function() {
    if (this.route_)
      this.route_.off('add', this.add, this);
  };
