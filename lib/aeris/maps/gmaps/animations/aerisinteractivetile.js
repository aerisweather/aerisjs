define(['aeris', 'base/animation'], function(aeris) {
   ...
   * @override
   */
  aeris.maps.gmaps.animations.AerisInteractiveTile.prototype.previous =
      function() {
    this.nextLayer_ = this.nextLayer_ - 1;
    if (this.nextLayer_ < 0)
      this.nextLayer_ = this.times_.length - 1;
    this.strategy_.hideLayer(this.times_[this.nextLayer_]);
    var setLayer = this.nextLayer_ - 1;
    if (setLayer < 0)
      setLayer = this.times_.length - 1;
    this.strategy_.setData('gmaps', this.layers_[this.times_[setLayer]]);
    this.strategy_.showLayer(this.times_[setLayer]);
  };
   ...
   * @override
   */
  aeris.maps.gmaps.animations.AerisInteractiveTile.prototype.next =
      function() {
    var setLayer = this.nextLayer_;
    var currLayer = this.nextLayer_ - 1;
    if (currLayer < 0)
      currLayer = this.times_.length - 1;
    this.nextLayer_ = (this.nextLayer_ + 1) % this.times_.length;
    this.strategy_.hideLayer(this.times_[currLayer]);
