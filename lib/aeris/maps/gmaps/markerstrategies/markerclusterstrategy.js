  ...
  */
  var MarkerClusterStrategy = function(object, opt_options) {
    ...
    this.listenTo(this.object_, {
      ...
      'change:clusterBy': function(obj, clusterBy, opts) {
        this.resetClusters();
      },
  ...
  MarkerClusterStrategy.prototype.addMarker = function(marker) {
    ...
    this.getClusterer(groupName).addMarker(marker.getView());
  };
  ...
  MarkerClusterStrategy.prototype.getMarkerGroup_ = function(marker) {
    var clusterBy = this.object_.getClusterBy();

    if (clusterBy) {
      return _.path(clusterBy, marker.toJSON());
    }

    // If no clusterBy is set,
    // use a single default cluster
    return MarkerClusterStrategy.SINGLE_CLUSTER_GROUPNAME;
  };
   ...
   */
  MarkerClusterStrategy.prototype.createClusterer_ = function(groupName, opt_markers) {
    var markers = opt_markers || [];

    // Convert Aeris Markers to Google marker views.
    var markerViews = _.map(markers, function(marker) {
      return marker.getView();
    });

    var clusterer = new this.MarkerClusterer_(
      this.mapView_,
      markerViews,
      {
        styles: this.object_.getClusterStyles(groupName)
      }
    );
