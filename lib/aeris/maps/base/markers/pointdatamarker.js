define([
  ...
  /**
   * Update marker position to
   * reflect PointData latLon.
   *
   * @private
   */
  PointDataMarker.prototype.updatePosition_ = function() {
    var latLon = this.get('data').get('latLon');
    this.setPosition(latLon);
  };


  /**
   * Updates the marker's title
   * based on PointData attributes.
   *
   * To set how the title is determined,
   * override #getTitle
   *
   * @private
   */
  PointDataMarker.prototype.updateTitle_ = function() {
    var title = this.getTitleFromData_();
