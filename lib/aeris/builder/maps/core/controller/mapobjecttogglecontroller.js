define([
...
], function(_, SimpleCollection, Marionette, Handlebars, toggleView) {
  /**
   ...
   * @param {aeris.SimpleCollection} opt_options.stateItems
   *
   * @constructor
   */
  var MapObjectToggleController = function(opt_options) {
    var options = _.extend({
      ...
      stateItems: new SimpleCollection()
    }, opt_options);
  ...
  MapObjectToggleController.prototype.bindStateToModel_ = function() {
    ...
    this.stateItems_.each(function(model) {
      if (model.getValue() === this.model.get('name')) {
        this.model.select();
      }
      else {
        this.model.deselect();
  ...
  MapObjectToggleController.prototype.bindModelToState_ = function() {
    this.listenTo(this.model, {
      select: function() {
        this.stateItems_.add(this.model.get('name'));
      },
      deselect: function() {
        this.stateItems_.remove(this.model.get('name'));
      }
    });
  };
