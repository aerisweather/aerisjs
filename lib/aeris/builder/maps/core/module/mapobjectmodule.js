define([
   ...
   *
   * @param {Object.<string,string>} options.labelLookup
   *        To lookup map object labels from map object class names.
   *        eg.
   *          { 'AerisRadSat': 'Radar & Satellite' }.
   *
   ...
   * @param {aeris.builder.maps.core.collection.MapObjectStateCollection} options.stateItems
  ...
  var MapObjectModule = function(options) {
    ...
    this.addInitializer(function(builderOptions) {
      ...
      _.each(toAdd, function(mapObjOpt) {
        this.stateItems_.add(_.omit(mapObjOpt, 'default'))
      }, this);
    });
  };
