define([
  ...
  MapAppBuilder.prototype.build = function() {
    var appOptions = {
      ...
      autoAnimate: this.getOption('autoAnimate')
    };

    var appController = new AppController(appOptions);

    appController.on('load', this.getOption('onload'));

    this.render_(appController);
  };
