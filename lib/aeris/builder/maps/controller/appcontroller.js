...
define([
  ...
  'aeris/builder/maps/controller/animationcontroller'
], function(
  _,
  Events,
  BaseController,
   ...
   * @param {Array.<aeris.maps.AbstractLayer>} options.layers Layer constuctors.
  ...
  AppController.prototype.initialize = function() {
    ...
    this.animationController_ = new AnimationController({
      layers: layers
    });
  };
  ...
  AppController.prototype.render = function() {
    ...
    if (this.options.controls.animation) {
      this.$el.append(this.animationController_.render().$el);
    }

    this.bindUIElements();
    this.delegateEvents();

    this.trigger('load', this.mapController_.getMap());
