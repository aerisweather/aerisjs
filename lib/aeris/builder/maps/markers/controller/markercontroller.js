define([
  ...
  var MarkerController = function() {
    ...
    this.listenTo(this.model, {
      // If marker is selected, but no filters are selected,
      // select all filters
      'select': function() {
        if (!this.model.get('filters').getSelected().length) {
          this.model.get('filters').invoke('select');
        }
      }
    });
  ...
  MarkerController.prototype.updateFilters_ = function() {
    ...
    this.view_.getParams().resetFilter(filterNames, { operator: 'OR' });
  };
