define([
  ...
  var MapAppBuilderOptions = function(opt_attrs, opt_options) {
     ...
     * @private
     */
    this.mapObjectTypes_ = opt_options.mapObjectTypes;
  ...
  MapAppBuilderOptions.prototype.validate = function(attrs) {
    // Check for invalid map object types
    var mapObjectOptions, invalidTypes;

    // Any keys besides those listed are
    // assumed to be map object modules
    mapObjectOptions = _.omit(attrs, 'el', 'map', 'autoAnimate', 'controls', 'onload', 'onerror');

    // Check for map object module keys
    invalidTypes = _.difference(_.keys(mapObjectOptions), this.mapObjectTypes_);

    // Check controls control for invalid map objects
    invalidTypes = invalidTypes.concat(_.difference(_.keys(attrs.controls), this.mapObjectTypes_));

    if (invalidTypes.length) {
      return new InvalidConfigError('Invalid map object module types: \'' + _(invalidTypes).unique().join('\', \'') + '\'.');
    }
  };
  ...
  MapAppBuilderOptions.prototype.normalizeMapObject_ = function(mapObjectOptions, opt_options) {
    ...
    // Return an empty array, if nothing's there
    if (!mapObjectOptions || !mapObjectOptions.length) {
      return [];
    }
    ...
    _.each(mapObjectOptions, function(opt, n) {
      ...
      if (_.isString(opt)) {
        mapObjectOptions[n] = _.extend({}, {
          name: opt
        }, defaultObj);
      }
      // Set defaults
      else {
        mapObjectOptions[n] = _.extend({}, defaultObj, opt);
      }
    }, this);
