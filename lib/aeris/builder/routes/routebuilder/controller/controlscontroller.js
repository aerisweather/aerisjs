define([
  ...
  'vendor/marionette/region'
], function(_, InvalidArgumentError, $, LayoutController, Region) {
  /**
   *
   ...
   * @param {Object} options.ui The following ui elements must be defined:
   *                 undo
   *                 redo
   *                 clear
   *                 collapse
  ...
  var ControlsController = function(options) {
    ...
    events['click ' + options.ui.undo] = this.undo;
    events['click ' + options.ui.redo] = this.redo;
    events['click ' + options.ui.clear] = _.bind(this.routeBuilder_.resetRoute, this.routeBuilder_, null);
    events['click ' + options.ui.collapse] = this.toggleShow;
    events['change' + options.ui.followDirections] = this.toggleFollowDirections;
    events['change ' + options.ui.latLonInputs] = this.setStartingWaypoint;
    events['keydown ' + options.ui.latLonInputs] = this.handleLatLonKeyPress_;
    ...
    // Event listeners not controlling to UI elements
    // can be set here, before the view is rendered.
  ...
  ControlsController.prototype.onRender = function() {
    // UI-bound event listeners are setup here.
